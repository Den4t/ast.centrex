...
(function($){
  ...
  $(function() {
    $('pre').floatingScrollbar();
  });

  //try to remove extra spaces from "Chan_Sip Peers" section
  $(function() {
	  $('h3').each(function(){
	      var elem = $(this);
	      if(elem.text() == 'Chan_Sip Peers') {
		var pre=elem.parent().next().children('pre');
		//console.log(pre.text());
		//pre.text(pre.text().replace(/^([^\s]+\s+[^\s]+)\s{25,25}/mg, '$1').replace(/\s+$/mg,''));
		pre.text(pre.text().replace(/^([^\s]+\s+[^\s]+)\s{22,22}/mg, '$1'));
	      }
	  });
  });
